...
**Content / file learning:** `learn file /tmp/opencode/re_requiem.txt` → topic extracted as **`Resident Evil Requiem`** (proper-noun title, not filename); **2048 neurons** (hard cap = `inter/4`, never full-FFN); 9 sentences; loss ≈ 1.5–1.9; sparse ckpt **~50 MB** for content+fact. Queries route via title match (`top1=1.00`) and answer by **retrieval-grounded generation**: the base LM answers from the expert's retrieved stored sentences under an exact-spelling + query-keyword gate, falling back verbatim to the grounded sentence when the generation misses a queried name/code (protagonist → Grace Ashcroft sentence; platforms → PS5/Xbox/PC sentence; multiplayer → co-op sentence). Foreign *memory-style* questions refuse; open-domain questions (Mars, meaning of life) route to base problem-solving (`answer_general`, rev. D).
...
- **Data (`src/gen_laya_evagi_data.py`)** — 2,897 train / 511 val / 53 held-out *tricky battery*: pronoun follow-ups ("Does it have multiplayer?" after a Requiem teach), hijack probes ("What's my blood type?" with unrelated memory), teach-vs-question boundaries, re-teach updates, origin/location frames ("Where am I from?" → `country` when stored, refuse otherwise), natural-case augmentation. Exclusion semantics forbid contradictions (`answer_from_memory` rows always carry a required topic; personal-memory rows exclude unrelated topics).
...
|-------|-----------|-----------|-------------------|
| Held-out val (n=511) | **1.000** | **1.000** | 26 ms / 41 ms |
| Tricky battery (n=53) | **0.981** (52/53) | 0.977 (42/43) | 26 ms / 47 ms |

Confidence is saturated (fitted T = 2.28), so the `conf < 0.55` System-2 fallback fires essentially only off-distribution — threshold sweep shows auto-coverage 1.000 / auto-precision 0.998 at any threshold 0–0.8. Residual error: 1/53 (capitalized exact-title phrasing). First call pays a one-time lazy load (~5–7 s); thereafter routing costs ~25–50 ms/turn.

**Integration (`src/live_interactive_lfm2.py`)** — S1 decision → existing Understanding mapping: `learn` → System-2 `extract_learn` value extraction (message-grounded parse first, LLM `field|value` line as backup; questions/`?`/pronoun/echo guards re-route to `understand()`), `answer_from_memory` → topic→eid map with embedding `match_expert` fallback — facts answered in a conversational frame (`Your name is Rehan.`), knowledge/content via retrieval-grounded generation over the expert's stored sentences — `admit_ignorance` → curiosity refusal, `answer_general`/`chitchat` → base generation. Every turn prints `system1: action=… conf=… (Xms)`; `understand()` survives only as fallback (low confidence or classifier unavailable — tested via `EVAGI_LAYA_DIR=/nonexistent`). **Behavior change:** open-domain questions (`What is the capital of France?`) are now answered by the base via `answer_general` instead of being refused; refusal is reserved for unlearned *personal/memory* queries.
...
3. **Base intelligence — behavior model (core of Layer 0)**  
   ...
   | **Learn-intent** | Turn any natural utterance into structured `learn\|kind\|value` (or answer/refuse/chat). | System-1 `laya-evagi` classifier → `learn` + `fact_kind`; System-2 `extract_learn` — message-grounded parse first (the utterance is ground truth for `my X is Y` / `I am from Z` frames), LLM `field\|value` line as backup; question/echo guards |
...
user prompt P
  → System-1: decide(P, known_memory)      # laya-evagi, ~25 ms (behavior #2)
      action=answer_from_memory:
          ...
          facts: conversational frame — "Your office code is ZEBRA-42."
          knowledge/content: retrieval-grounded generation over e's stored
            sentences (spelling-exact + query-keyword gate; raw sentence as
            faithful fallback)
...
5. **Content answers** use retrieval-grounded generation over stored sentences with an exact-spelling/keyword gate and verbatim retrieval as the faithful fallback; book-scale recall not solved.  
...
- **Routing is learned, not scripted** (rev. D): a fine-tuned System-1 classifier (`laya-evagi`) replaced every regex and the intent prompt in the turn path — 100% action accuracy on 511 held-out decisions, 98% on a 53-case adversarial battery, ~26 ms/turn (vs. ~0.5–2 s for an LLM router). Curiosity, learn-intent, and open-domain problem-solving are now *trained dispositions* of Layer 0.  
